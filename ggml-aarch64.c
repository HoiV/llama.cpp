...
//
// Additional quant block definitions (see ggml-common.h for others - block_q4_0x4...)
...
void quantize_q8_K_4x8(const float * GGML_RESTRICT x, void * GGML_RESTRICT vy, int64_t k) {
    assert(QK_K == 256);
    assert(k % QK_K == 0);
    const int nb = k / QK_K;
...
#endif ! ((defined(_MSC_VER)) && ! defined(__clang__)) && defined(__aarch64__)
}
...
bool ggml_aarch64_repack_tensor(struct ggml_tensor * cur, enum ggml_type repack_type, const void * restrict data, size_t data_size) {
    ...
    if (cur->type == GGML_TYPE_Q4_0) {
        if (repack_type == GGML_TYPE_Q4_0_8_8) {
            if (repack_q4_0_to_q4_0_8_bl(cur, 8, data, data_size) != 0) {
                return false;
            }
        }
    } else if (cur->type == GGML_TYPE_Q4_K) {
        if (repack_type == GGML_TYPE_Q4_K_8_8) {
            if (repack_q4_K_to_q4_K_8_bl(cur, 8, data, data_size) != 0) {
                return false;
            }
        }
    } else {
        GGML_ASSERT("Unsupported type");
    }
        ...
        for (int64_t x = 0; x < nb; x++) {
            ...
            if (nrows_interleaved == 8) {
                *(block_q4_0x8 *) out_ptr = make_block_q4_0x8(dst_tmp, blck_size_interleave);
                ...
                GGML_ASSERT("Unsupported Q4_0_4_X format type");
