/* djtext.c - DJGPP front end, text functions
 *	Original BCtext.c Copyright (c) 1995-1997 Stefan Jokisch
...
void load_fonts (void)
{
    ...
    } else graphics_font = font_data + chunk_offset[0];
...
void os_display_char (zchar c)
{
	...
	if (current_font == GRAPHICS_FONT) {
	    ...
	    dos_table = RM_TO_LINEAR(dos_table) + h_font_height * c;
	    mask = 0xff;
	...
	for (i = 0; i < h_font_height; i++) {
	    ...
	    if (type == 1)
		val = *((byte *) table + 8 * i / h_font_height);
	    if (type == 2)
		val = *((word *) table + i);
	    if (type == 3)
		val = *((byte *) table + i);
