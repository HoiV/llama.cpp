/* djscreen.c - DJGPP front end, screen manipulation
 *	Original BCscreen.c Copyright (c) 1995-1997 Stefan Jokisch
...
static void copy_byte (unsigned long scrn1, unsigned long scrn2, byte mask)
{
	...
	_farnspokeb(scrn1, _farnspeekb(scrn1) & ~mask
			 | _farnspeekb(scrn2) & mask);
...
static void copy_line (int y1, int y2, int left, int right)
{
	...
	movedata(_dos_ds, scrn2 + left, _dos_ds, scrn1 + left,
	    right - left + 1);
