...
int slm_init(xbapp_params& params) {
    ...
    if (params.pfc_mode) {
        ...
        size_t pos = template_prompt.find("{CURRENT_SETTING}");
        if (pos != std::string::npos) {
            ...
            printf("%s: Using shared prompt - size %zd\n", __func__, tokens_shared.size());
            ...
            if (!llama_state_load_file(ctx, 
                ...
                for (size_t i = 0; i < tokens_shared.size(); i++) {
                    if (tokens_shared[i] != session_tokens[i]) {
                        printf("Mismatched pfx tokens [%zd] - %4X %4X %4X %4X - %4X %4X %4X %4X !!!!\n"
                               "      tokens_shared size = %zd - session_tokens size = %zd", i, 
                                tokens_shared[i-2], tokens_shared[i-1], tokens_shared[i], tokens_shared[i+1],
                                session_tokens[i-2], session_tokens[i-1], session_tokens[i], session_tokens[i+1],
                                tokens_shared.size(), session_tokens.size());
                        return 1;
                    }
                }
