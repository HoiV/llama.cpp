...
int main(int argc, char** argv) {
    ...
    size_t setting_index = full_prompt.find("{CurrentSetting}");
    if (setting_index == std::string::npos) {
        printf("%s: template prompt is not correctly formed for pfc mode - "
               "no \"{CurrentSetting}\" identifier located\n", __func__);
        if (xbparams.pfc_mode) {
            return 1;
        }
    }
    // for now just replace "{CurrentSetting}" with nothing
    full_prompt.replace(setting_index, 
        std::string("{CurrentSetting}").length(), "");
