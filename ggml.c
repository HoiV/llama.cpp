...
typedef volatile LONG64 atomic_int64;
...
inline LONG atomic_fetch_add(atomic_int * ptr, LONG inc) {
    return InterlockedExchangeAdd(ptr, inc);
}
inline LONG64 atomic_fetch_add64(atomic_int64 * ptr, LONG64 inc) {
    return InterlockedExchangeAdd64(ptr, inc); 
}
inline LONG atomic_fetch_sub(atomic_int * ptr, LONG dec) {
    return atomic_fetch_add(ptr, -(dec));
}
inline atomic_bool atomic_flag_test_and_set(atomic_flag * ptr) {
    return InterlockedExchange(ptr, 1);
}
inline void atomic_flag_clear(atomic_flag * ptr) {
...
#define GGML_F32x8_MAX     _mm256_max_ps
...
#define GGML_F32x8_XOR     _mm256_xor_ps
...
#define GGML_F32x8_REDUCE_MAX(max, x)                             \
do {                                                              \
    int offset = GGML_F32_ARR >> 1;                               \
    for (int64_t i = 0; i < offset; ++i) {                        \
        x[i] = _mm256_max_ps(x[i], x[offset+i]);                  \
    }                                                             \
    offset >>= 1;                                                 \
    for (int64_t i = 0; i < offset; ++i) {                        \
        x[i] = _mm256_max_ps(x[i], x[offset+i]);                  \
    }                                                             \
    const __m128 t0 = _mm_max_ps(_mm256_castps256_ps128(x[0]),    \
                                 _mm256_extractf128_ps(x[0], 1)); \
...
#define GGML_F32_VEC_LOAD128 GGML_F32x4_LOAD
#define GGML_F32_VEC_STORE128 GGML_F32x4_STORE 
...
#define GGML_F32_EPR16 16

#define GGML_F32_VEC512      __m512
#define GGML_F32_VEC_ZERO512 _mm512_setzero_ps()
#define GGML_F32_VEC_LOAD512 _mm512_loadu_ps
#define GGML_F32_VEC_FMA512(a, b, c) _mm512_fmadd_ps(b, c, a)
#define GGML_F32_VEC_ADD512  _mm512_add_ps
#define GGML_F32_VEC_MUL512  _mm512_mul_ps
#define GGML_F32_VEC_STORE512 _mm512_storeu_ps
#define GGML_F32_VEC_PREFETCH(a) _mm_prefetch((char const *)(a), _MM_HINT_T2)
...
#ifdef __AVX512F__
#define GGML_F16_STEP16 64
#define GGML_F16_EPR16 16

#define GGML_F16_VEC512       __m512
#define GGML_F16_VEC_ZERO512  _mm512_setzero_ps()
#define GGML_F16_VEC_LOAD512(x)  _mm512_cvtph_ps(_mm256_loadu_si256((__m256i const *)(x)))
#define GGML_F16_VEC_FMA512(a, b, c) _mm512_fmadd_ps(b, c, a)

#define GGML_F16_VEC_REDUCE512(res, x) GGML_F32_VEC_REDUCE512(res, x)
#endif // __AVX512F__
...
void ggml_vec_add_f32(const int32_t n, float * z, const float * x, const float * y)
{
#ifdef GGML_SIMD
    int64_t i = 0;
...
void ggml_vec_set_f32(const int64_t n, float * x, const float v)
{
#ifdef GGML_SIMD
    int64_t i = 0;
    const int64_t xn = (n & ~(GGML_F32_EPR - 1));
...
void ggml_vec_dot_f32(const int n, float * restrict s, size_t bs, const float * restrict x, size_t bx, const float * restrict y, size_t by, int nrc) {
   ...
   UNUSED(bs);
    ...
    if (xn) {
        GGML_F32_VEC512 sum[GGML_F32_ARR];
        GGML_F32_VEC512 ax[GGML_F32_ARR];
        GGML_F32_VEC512 ay[GGML_F32_ARR];
        const int64_t np = (n & ~(GGML_F32_STEP16 - 1));

        for (int64_t j = 0; j < GGML_F32_ARR; j++) {
            sum[j] = GGML_F32_VEC_ZERO512;
        }
        ...
        if (np) {
            do {
              for (int64_t j = 0; j < GGML_F32_ARR; j++) {
                  ax[j] = GGML_F32_VEC_LOAD512(x + i + j * GGML_F32_EPR16);
                  ay[j] = GGML_F32_VEC_LOAD512(y + i + j * GGML_F32_EPR16);
                  sum[j] = GGML_F32_VEC_FMA512(sum[j], ax[j], ay[j]); 
              }
        ...
        if (xn > np) {
            do {
                ax[0] = GGML_F32_VEC_LOAD512(x + i);
                ay[0] = GGML_F32_VEC_LOAD512(y + i);
                sum[0] = GGML_F32_VEC_FMA512(sum[0], ax[0], ay[0]);
                i += GGML_F32_EPR16;
            } while (i < xn);
        }
...
void ggml_vec_dot_f16(const int n, float * restrict s, size_t bs, const ggml_fp16_t * restrict x, size_t bx, const ggml_fp16_t * restrict y, size_t by, int nrc) {
    ...
    if (xn) {
        GGML_F16_VEC512 sum[GGML_F16_ARR];
        GGML_F16_VEC512 ax[GGML_F16_ARR];
        ...
        if (np) {
            do {
                for (int64_t j = 0; j < GGML_F16_ARR; j++) {
                    ax[j] = GGML_F16_VEC_LOAD512(x + i + j * GGML_F16_EPR16);
                    ay[j] = GGML_F16_VEC_LOAD512(y + i + j * GGML_F16_EPR16);
                    sum[j] = GGML_F16_VEC_FMA512(sum[j], ax[j], ay[j]);
                }
        ...
        if (xn > np) {
            do {
                ax[0] = GGML_F16_VEC_LOAD512(x + i);
                ay[0] = GGML_F16_VEC_LOAD512(y + i);
                sum[0] = GGML_F16_VEC_FMA512(sum[0], ax[0], ay[0]);
                i += GGML_F16_EPR16;
            } while (i < xn);
        }
...
void ggml_vec_dot_f16_f32(const int64_t n, float * restrict s, size_t bs, const ggml_fp16_t * restrict x, size_t bx, const float * restrict y, size_t by, int nrc) {
    ...
    if (xn) {
        ...
        if (np) {
            do {
                for (int64_t j = 0; j < GGML_F32_ARR; j++) {
                    av[j] = _mm256_loadu_si256((__m256i *)(x + i + j * GGML_F32_EPR16));
                    ax[j] = _mm512_cvtph_ps(av[j]);
                    ay[j] = GGML_F32_VEC_LOAD512(y + i + j * GGML_F32_EPR16);
                    sum[j] = GGML_F32_VEC_FMA512(sum[j], ax[j], ay[j]);
                }
        ...
        if (xn > np) {
            do {
                av[0] = _mm256_loadu_si256((__m256i *)(x + i));
                ax[0] = _mm512_cvtph_ps(av[0]);
                ay[0] = GGML_F32_VEC_LOAD512(y + i);
                sum[0] = GGML_F32_VEC_FMA512(sum[0], ax[0], ay[0]);
                i += GGML_F32_EPR16;
            } while (i < xn);
...
atomic_int64 vec_dot16_time = 0;
atomic_int64 vec_dot32_counts = 0;
atomic_int64 vec_dot32_time = 0;
...
#ifdef GGML_VECTOR_DOT_PERF
    printf(" Vector Dot Performance\n\n");
    printf("Total count is the number of FMA operations divided by the vector width\n\n");
    if (vec_dot16_counts) {
        double average = (double)((vec_dot16_time * 100) / vec_dot16_counts) / 100.;
        printf("vec_dot16 total count  %I64u\n", (uint64_t)vec_dot16_counts);
        printf("          total time   %I64u\n", (uint64_t)vec_dot16_time);
        printf("          average time in cycles %5.2f\n\n", average);
    }

    if (vec_dot32_counts) {
        double average = (double)((vec_dot32_time * 100) / vec_dot32_counts) / 100.;  
        printf("vec_dot32 total count  %I64u\n", (uint64_t)vec_dot32_counts);
        printf("          total time   %I64u\n", (uint64_t)vec_dot32_time);
        printf("          average time in cycles %5.2f\n\n", average);
    }
#endif // GGML_VECTOR_DOT_PERF
...
// * At best  it crashes or talks nonsense.
// * At worst it talks slightly differently but hard to perceive.
//
...
//
// N.B. GGML_OP_HAS_INIT and GGML_OP_HAS_FINALIZE are deprecated. Previously these designations
//      were contained in and controlled by the tensor compute dispatcher. This led to additional
//      overhead in the dispatcher for all tensor computation. The init and finalize operations
...
inline static void ggml_critical_section_end(void) {
    atomic_flag_clear(&g_state_barrier);
}
...
extern inline GGML_CALL int64_t ggml_nelements(const struct ggml_tensor * tensor) {
    static_assert(GGML_MAX_DIMS == 4, "GGML_MAX_DIMS is not 4 - update this function");

    return tensor->ne[0]*tensor->ne[1]*tensor->ne[2]*tensor->ne[3];
}

extern inline int64_t ggml_nrows(const struct ggml_tensor * tensor) {
    static_assert(GGML_MAX_DIMS == 4, "GGML_MAX_DIMS is not 4 - update this function");
...
extern inline size_t ggml_nbytes_pad(const struct ggml_tensor * tensor) {
    return GGML_PAD(ggml_nbytes(tensor), GGML_MEM_ALIGN);
}

extern inline size_t ggml_blck_size(enum ggml_type type) {
    return type_traits[type].blck_size;
}

extern inline size_t ggml_type_size(enum ggml_type type) {
    return type_traits[type].type_size;
}

extern inline size_t ggml_row_size(enum ggml_type type, int64_t ne) {
    assert(ne % ggml_blck_size(type) == 0);
    return ggml_type_size(type)*ne/ggml_blck_size(type);
}

extern inline double ggml_type_sizef(enum ggml_type type) {
    return ((double)(type_traits[type].type_size))/type_traits[type].blck_size;
}

extern inline const char * ggml_type_name(enum ggml_type type) {
...
extern inline const char * ggml_op_symbol(enum ggml_op op) {
    return GGML_OP_SYMBOL[op];
}

extern inline const char * ggml_unary_op_name(enum ggml_unary_op op) {
...
extern inline size_t ggml_element_size(const struct ggml_tensor * tensor) {
    return ggml_type_size(tensor->type);
}

extern inline bool ggml_is_scalar(const struct ggml_tensor * tensor) {
    static_assert(GGML_MAX_DIMS == 4, "GGML_MAX_DIMS is not 4 - update this function");

    return tensor->ne[0] == 1 && tensor->ne[1] == 1 && tensor->ne[2] == 1 && tensor->ne[3] == 1;
}

extern inline bool ggml_is_vector(const struct ggml_tensor * tensor) {
    static_assert(GGML_MAX_DIMS == 4, "GGML_MAX_DIMS is not 4 - update this function");

    return tensor->ne[1] == 1 && tensor->ne[2] == 1 && tensor->ne[3] == 1;
}

extern inline bool ggml_is_matrix(const struct ggml_tensor * tensor) {
    static_assert(GGML_MAX_DIMS == 4, "GGML_MAX_DIMS is not 4 - update this function");
...
extern inline int ggml_n_dims(const struct ggml_tensor * tensor) {
...
static bool ggml_is_contiguous_n(const struct ggml_tensor * tensor, int n) {
    ...
    if (tensor->ne[0] != (int64_t)ggml_blck_size(tensor->type) && tensor->nb[0] != next_nb) {
        return false;
    }
...
static struct ggml_tensor * ggml_new_tensor_impl(
    ...
    result->type = type;
    result->backend =GGML_BACKEND_TYPE_CPU;

    for (int64_t i = 0; i < GGML_MAX_DIMS; i += 1) {
        result->ne[i] = 1;
    }
...
extern inline struct ggml_tensor * ggml_new_tensor(
        struct ggml_context * ctx,
        enum   ggml_type      type,
        int                   n_dims,
        const int64_t       * ne) {
...
extern inline struct ggml_tensor * ggml_new_tensor_1d(
        struct ggml_context * ctx,
        enum   ggml_type      type,
        int64_t ne0) {
...
extern inline struct ggml_tensor * ggml_new_tensor_2d(
        struct ggml_context * ctx,
        enum   ggml_type      type,
        int64_t ne0,
        int64_t ne1) {
...
extern inline struct ggml_tensor * ggml_new_tensor_3d(
        struct ggml_context * ctx,
        enum   ggml_type      type,
        int64_t ne0,
        int64_t ne1,
...
extern inline struct ggml_tensor * ggml_new_tensor_4d(
        struct ggml_context * ctx,
        enum   ggml_type type,
        int64_t ne0,
        int64_t ne1,
...
inline int32_t ggml_get_op_params_i32(const struct ggml_tensor * tensor, uint32_t i) {
    assert(i < GGML_MAX_OP_PARAMS / sizeof(int32_t));
    return ((const int32_t *)(tensor->op_params))[i];
}

inline float ggml_get_op_params_f32(const struct ggml_tensor * tensor, uint32_t i) {
    assert(i < GGML_MAX_OP_PARAMS / sizeof(float));
    return ((const float *)(tensor->op_params))[i];
}

inline void ggml_set_op_params_i32(struct ggml_tensor * tensor, uint32_t i, int32_t value) {
    assert(i < GGML_MAX_OP_PARAMS / sizeof(int32_t));
    ((int32_t *)(tensor->op_params))[i] = value;
}

inline void ggml_set_op_params_f32(struct ggml_tensor * tensor, uint32_t i, float value) {
    assert(i < GGML_MAX_OP_PARAMS / sizeof(float));
...
extern inline enum ggml_unary_op ggml_get_unary_op(const struct ggml_tensor * tensor) {
...
void ggml_compute_forward_dup_same_cont(
...
void ggml_compute_forward_add_f32(
    ...
    int64_t r1 = ne02 * ne01;

    if (nb10 == sizeof(float)) {
        int64_t nr0 = ne00 / ne10;
        for (int64_t ir = ir0; ir < ir1; ++ir) {
            // src1 is broadcastable across src0 and dst in i1, i2, i3
            const int64_t i03 = ir / r1;
            const int64_t i02 = (ir - i03 * r1) / ne01;
            const int64_t i01 = (ir - i03 * r1 - i02 * ne01);
...
void ggml_compute_forward_add_f16_f32(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
...
void ggml_compute_forward_add_f16_f16(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
...
void ggml_compute_forward_add1_f32(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
...
void ggml_compute_forward_add1(
    ...
    const enum ggml_type src0_type = dst->src[0]->type;
    const enum ggml_type src1_type = dst->src[1]->type;
...
void ggml_compute_forward_sum_f16(
    const struct ggml_compute_params * params,
          struct ggml_tensor * dst) {
    UNUSED(params);
    ...
    for (int64_t i03 = 0; i03 < ne03; i03++) {
        for (int64_t i02 = 0; i02 < ne02; i02++) {
            for (int64_t i01 = 0; i01 < ne01; i01++) {
                ...
                *(float *) ((char *) dst->data + i01*nb1 + i02*nb2 + i03*nb3) *= recip;
            }
        }
    }
}
...
void ggml_compute_forward_argmax_f32(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    UNUSED(params);
...
void ggml_compute_forward_concat_f32(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
    UNUSED(params);
...
void ggml_compute_forward_leaky_relu_f32(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    UNUSED(params);
...
void ggml_compute_forward_silu_back_f32(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
...
void ggml_compute_forward_hardswish_f32(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    UNUSED(params);
...
void ggml_compute_forward_hardsigmoid_f32(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    UNUSED(params);
...
void ggml_compute_forward_mul_mat(
    ...
    //00000045`079ee1a0 00007ff6`7a27e1ba     batchslm!ggml_compute_forward_mul_mat+0x18d [E:\Xbox-B612\src\llama.dc_matmul\ggml.c @ 13446]
    //00000045`079ee350 00007ff6`7a27df02     batchslm!ggml_graph_compute_thread+0x18a [E:\Xbox-B612\src\llama.dc_matmul\ggml.c @ 20288]
    //00000045`079ee3b0 00007ff6`7a363646     batchslm!ggml_graph_compute+0x202 [E:\Xbox-B612\src\llama.dc_matmul\ggml.c @ 20637]
    //00000045`079ee570 00007ff6`7a3652bc     batchslm!ggml_backend_cpu_graph_compute+0xb6 [E:\Xbox-B612\src\llama.dc_matmul\ggml-backend.c @ 812]
    //00000045`079ee5f0 00007ff6`7a2e0182     batchslm!ggml_backend_sched_compute_splits+0x14c [E:\Xbox-B612\src\llama.dc_matmul\ggml-backend.c @ 1777]
    //00000045`079ee700 00007ff6`7a325eaf     batchslm!llama_decode_internal+0x872 [E:\Xbox-B612\src\llama.dc_matmul\llama.cpp @ 12546]
    //00000045`079ee910 00007ff6`7a1e6de6     batchslm!llama_decode+0x2f [E:\Xbox-B612\src\llama.dc_matmul\llama.cpp @ 18343]
    //00000045`079ee990 00007ff6`7a3893d0     batchslm!main+0xa46 [E:\Xbox-B612\src\llama.dc_matmul\examples\batchslm\batchslm.cpp @ 408]
    //(Inline Function) --------`--------     batchslm!invoke_main+0x22 [D:\a\_work\1\s\src\vctools\crt\vcstartup\src\startup\exe_common.inl @ 78]
    //00000045`079ef760 00007ff8`0381257d     batchslm!__scrt_common_main_seh+0x10c [D:\a\_work\1\s\src\vctools\crt\vcstartup\src\startup\exe_common.inl @ 288]
...
#ifdef GGML_VECTOR_DOT_PERF 
    atomic_int64 dot_count16 = 0;
    atomic_int64 dot_time16 = 0;
    atomic_int64 dot_count32 = 0;
...
//    printf("ne00 %lld, block_factor %lld\n", ne00, blck_factor);
            ...
            for (int64_t ir1 = iir1; ir1 < limit1; ++ir1) {
                ...
                for (int64_t ir0 = iir0; ir0 < limit0; ++ir0) {
                    ...
                    if (vec_dot_type == GGML_TYPE_F32) {
                        int64_t t_start = __rdtsc();
                        ggml_vec_dot_f32(ne00,
                                         &dst_col[ir0], 0,
                                         (const float *)(src0_row + ir0*nb01), 0,
                                         (const float *)src1_col, 0, 1);
                        dot_time32 += __rdtsc() - t_start;
                        dot_count32 += ne00;

                    } else if (vec_dot_type == GGML_TYPE_F16) {
                        int64_t t_start = __rdtsc();
                        ggml_vec_dot_f16(ne00,
                                         &dst_col[ir0], 0
                                         (const ggml_fp16_t *)(src0_row + ir0*nb01), 0,
                                         (const ggml_fp16_t *)src1_col, 0, 1);
                        dot_time16 += __rdtsc() - t_start;
                        dot_count16 += ne00;
    ...
    if (vec_dot_type == GGML_TYPE_F32) {
        ...
        dot_count32 /= 8;
#endif // __AVX512F__
        atomic_fetch_add64(&vec_dot32_counts, dot_count32);
        atomic_fetch_add64(&vec_dot32_time, dot_time32);
        ...
        dot_count16 /= 8;
#endif // __AVX512F__
        atomic_fetch_add64(&vec_dot16_counts, dot_count16);
        atomic_fetch_add64(&vec_dot16_time, dot_time16);
    }
#endif // GGML_VECTOR_DOT_PERF 
}
...
void ggml_compute_forward_get_rows_back_f32_f16(
        const struct ggml_compute_params * params,
              struct ggml_tensor * dst) {
...
void ggml_compute_forward_get_rows_back_f32(
        const struct ggml_compute_params * params,
              struct ggml_tensor * dst) {
...
void ggml_compute_forward_diag_f32(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    UNUSED(params);
...
void ggml_compute_forward_im2col_f32(
        const struct ggml_compute_params * params,
              struct ggml_tensor * dst) {
...
// src1: image [N, IC, IH, IW]
// dst:  result [N, OH, OW, IC*KH*KW]
void ggml_compute_forward_im2col_f16(
        const struct ggml_compute_params * params,
              struct ggml_tensor * dst) {
...
void ggml_compute_forward_pool_1d_sk_p0(
        const struct ggml_compute_params * params,
...
void ggml_compute_forward_upscale_f32(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
...
void ggml_compute_forward_pad_f32(
    const struct ggml_compute_params * params,
          struct ggml_tensor * dst) {
...
void ggml_compute_forward_flash_attn_ext_f16(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
...
const op_func ggml_unary_op_dispatch[] = {
    [GGML_UNARY_OP_ABS] = &ggml_compute_forward_abs,
    [GGML_UNARY_OP_SGN] = &ggml_compute_forward_sgn,
    [GGML_UNARY_OP_NEG] = &ggml_compute_forward_neg,
    [GGML_UNARY_OP_STEP] = &ggml_compute_forward_step,
    [GGML_UNARY_OP_TANH] =  &ggml_compute_forward_tanh,
    [GGML_UNARY_OP_ELU] =  &ggml_compute_forward_elu,
    [GGML_UNARY_OP_RELU] =  &ggml_compute_forward_relu,
    [GGML_UNARY_OP_GELU] =  &ggml_compute_forward_gelu,
    [GGML_UNARY_OP_GELU_QUICK] = &ggml_compute_forward_gelu_quick,
    [GGML_UNARY_OP_SILU] =  &ggml_compute_forward_silu,
    [GGML_UNARY_OP_HARDSWISH] =  &ggml_compute_forward_hardswish,
    [GGML_UNARY_OP_HARDSIGMOID] =  &ggml_compute_forward_hardsigmoid
};

static_assert(GGML_UNARY_OP_COUNT == ARRAYSIZE(ggml_unary_op_dispatch), "unary op count mismatch");
...
static void ggml_compute_forward_unary(
    ...
    GGML_ASSERT(op < GGML_UNARY_OP_COUNT);

    ggml_unary_op_dispatch[op](params, dst);
...
const op_func ggml_compute_op_dispatch[] = {
    [GGML_OP_NONE] = &ggml_compute_forward_nop,
    ...
    [GGML_OP_SQRT] = &ggml_compute_forward_sqrt,
    ...
    [GGML_OP_NORM] = &ggml_compute_forward_norm,
    ...
    [GGML_OP_SOFT_MAX_BACK] = &ggml_compute_forward_soft_max_back,
    ...
    [GGML_OP_UPSCALE] = &ggml_compute_forward_upscale,
...
int ggml_get_n_tasks(struct ggml_tensor * node, int n_threads) {
    ...
    case GGML_OP_MUL_MAT_ID:
    case GGML_OP_OUT_PROD:
    case GGML_OP_SCALE: // XBOX_INVESTIGATE (n_tasks = 1)
    case GGML_OP_SET: // XBOX_INVESTIGATE (n_tasks = 1)
    case GGML_OP_GET_ROWS: // XBOX_INVESTIGATE (n_tasks = MIN(n_cur_threads, ggml_nelements(node->src[1]));)
    case GGML_OP_DIAG_MASK_ZERO:
    case GGML_OP_DIAG_MASK_INF:
    case GGML_OP_SOFT_MAX_BACK:
    case GGML_OP_ROPE:
    ...
    case GGML_OP_MAP_CUSTOM1:
        {
            ...
            if (p->n_tasks == GGML_N_TASKS_MAX) {
                ...
                return MIN(p->n_tasks, n_threads);
            }
        }
    ...
    // compute the total size of the data section, taking into account the alignment
    {
        ...
        for (uint64_t i = 0; i < ctx->header.n_tensors; ++i) {
            ...
            if (ne % ggml_blck_size(info->type) != 0) {
                fprintf(stderr, "%s: tensor '%s' of type %d (%s) number of elements (%" PRId64 ") is not a multiple of block size (%zd)\n",
