...
#pragma comment(linker, "/EXPORT:ggml_time_init=" __FUNCTION__)
    if (!timer_freq) {
        LARGE_INTEGER t;
...
static const ggml_type_traits_t type_traits[GGML_TYPE_COUNT] = {
    ...
    [GGML_TYPE_Q3_K] = {
        ...
        .from_float_reference     = (ggml_from_float_t) quantize_row_q3_K,
        .vec_dot                  = (ggml_vec_dot_t) ggml_vec_dot_q3_K_q8_K,
        .vec_dot_type             = GGML_TYPE_Q8_K,
        .nrows                    = 1,
    },
    [GGML_TYPE_Q4_K] = {
    ...
    [GGML_TYPE_Q6_K] = {
        ...
        .from_float_reference     = (ggml_from_float_t) quantize_row_q6_K,
...
#ifdef __AVX2__ // This covers both AVX512 as well as AVX2
...
float ggml_cosine_similarity_f32(const int n, const float *x,  const float *y) {
...
float ggml_cosine_similarity_bf16(const int n, const ggml_bf16_t *x, const ggml_bf16_t *y) {
...
void ggml_vec_mad_f16(const uint64_t n, ggml_fp16_t * restrict y, const ggml_fp16_t * restrict x, const float v) {
...
void ggml_vec_max_f32(const int32_t n, float * s, const float * x) {
    ...
    )
{
    ...
    if (n_tasks != 1) {
        ...
        //
        // Compute the barrier wait end time and update the barrier wait statistics.
        //
        ...
        if (src1_type != vec_dot_type) {
            ...
            for (int64_t i13 = 0; i13 < ne13; ++i13) {
                for (int64_t i12 = 0; i12 < ne12; ++i12) {
                    ...
                    if ((ggml_n_dims(src1) == 2) && from_float_to_mat) {
    ...
    for (uint32_t node_n = 0; node_n < graph_n_nodes; node_n++) {
        ...
        if (ith < n_tasks) {
            ...
            if (!ith) {
                ...
                //
                // Update tensor op counts and op time.
                //
        ...
        if (n_threads != 1) {
            ...
            //
            // Compute tensor wait start time.
            //
            // N.B. The tensor wait time computation is not perfectly synchronized, but
            //      it is close and provides meaningful data. The computation is performed
            //      by the zeroth thread which is always involved in the computation of
            //      the tensor and does maximum work.
            //
            ...
            int generation_old = *generation;
            if (atomic_fetch_add(barrier, 1) == (n_threads - 1)) {
                ...
                *barrier = 0;
                ...
                //
                // Wait for remaining threads to arrive.
                //
            ...
            //
            // Compute the tensor wait end time and update the tensor wait statistics.
            //
...
enum ggml_status ggml_graph_compute(struct ggml_cgraph * cgraph, struct ggml_cplan * cplan) {
    ...
    if (ggml_use_omp) {
        ...
        //
        // The current thread is also a worker thread.
        //
