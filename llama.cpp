...
void llama_print_timings(struct llama_context * ctx) {
    ...
    LLAMA_LOG_INFO("%s:      sample time = %10.2f ms / %5d runs   (%8.2f ms per token, %8.2f tokens per second)\n",
            ...
            (timings.n_sample == 0) ? 0.0 : timings.t_sample_ms / timings.n_sample, 
            (timings.t_sample_ms == 0) ? 0.0 : 1e3 / timings.t_sample_ms * timings.n_sample); 
    LLAMA_LOG_INFO("%s: prompt eval time = %10.2f ms / %5d tokens (%8.2f ms per token, %8.2f tokens per second)\n",
            __func__, timings.t_p_eval_ms, timings.n_p_eval, 
            (timings.n_p_eval == 0) ? 0.0: timings.t_p_eval_ms / timings.n_p_eval, 
            (timings.t_p_eval_ms == 0) ? 0.0 : 1e3 / timings.t_p_eval_ms * timings.n_p_eval);
    LLAMA_LOG_INFO("%s:        eval time = %10.2f ms / %5d runs   (%8.2f ms per token, %8.2f tokens per second)\n",
            __func__, timings.t_eval_ms, timings.n_eval, 
            (timings.n_eval == 0) ? 0.0 : timings.t_eval_ms / timings.n_eval, 
            (timings.t_eval_ms == 0) ? 0.0 : 1e3 / timings.t_eval_ms * timings.n_eval);
